# Detail Design Document (DLD): Issue Documents Feature

## Overview

This document outlines the detailed design for implementing the Issue Documents feature in Paperclip. This feature addresses the immediate workflow problem where plans are embedded in issue descriptions as `<plan>` blocks, by providing first-class, editable, revisioned text documents that can be linked to issues.
...
- **Date**: 2026-06-23
- **Primary Issue**: ATA-36

## Summary

Add first-class **documents** to Paperclip as editable, revisioned, company-scoped text artifacts that can be linked to issues. The first required convention is a document with key `plan`.

This solves the immediate workflow problem:
...
- optionally add explicit import/migrate path later
...
This DLD requires approval from:

- CTO (for technical feasibility)
- CEO (for strategic alignment)
- Board (for resource allocation)

## Next Steps

1. Get approval for this DLD
2. Create implementation subtasks for each phase
3. Begin Phase 1: Shared contract and schema
4. Coordinate with backend, UI, and agent protocol teams
5. Update the issue status to `in_review` once DLD is approved

## Related Documents

- [Issue Documents Plan](/docs/plans/2026-03-13-issue-documents-plan.md)
- [Paperclip API Reference](/skills/paperclip/references/api-reference.md)
- [Artifact System Plan](/docs/plans/artifact-system-plan.md)
