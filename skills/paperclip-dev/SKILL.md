...
## Repository Policy — Read This First

> **STOP: Paperclip agents MUST NOT fix or modify Paperclip's own code.**
> Paperclip is the infrastructure the agents run on, not a project they work on. If you are an agent
> running *inside* Paperclip and a task asks you to change the control plane, its adapters, its
> server/ui/cli, or its migrations — **do not do it.** Report it back and stop. Self-modification of
> the platform by the agents it schedules has repeatedly broken the running instance. Changes to
> Paperclip are made deliberately by a human operator, not as agent task output.

> **"The fork" always means `github.com/atarim-info/paperclip`.**
> Not `atarim-info/utms`. Not `paperclipai/paperclip`. When instructed to "work with the fork", use
> `atarim-info/paperclip` — no interpretation, no substitution.

**Required remote layout** in the working checkout:

| Remote | URL | Role |
|---|---|---|
| `origin` | `github.com/atarim-info/paperclip` | **the fork** — all day-to-day work, all branches, all PRs |
| `upstream` | `github.com/paperclipai/paperclip` | canonical Paperclip — fetch-only source of updates |

Verify with `git remote -v` before any branch, push, or PR operation. If `origin` points anywhere
else — especially at a non-Paperclip repo — **stop and fix the remote first.** A `git pull origin
master` against the wrong repo silently replaces the tree with a foreign trunk; this has happened
and cost a day of debugging.

**Never put Paperclip code in `atarim-info/utms`.** That repo is the UTMS Java/Spring platform.
Paperclip branches were once pushed there, creating a second unrelated-history trunk on its
`master`; that work has since been migrated to the fork under `salvage/*` and removed. Do not
recreate it. ATA-* ticket ids are shared across both products — a UTMS-sounding ticket name does
**not** mean the branch belongs in the UTMS repo. Check what the branch actually changes.

**Keep the fork current** with upstream, and **contribute back**: fork-only work of general value
(local CLI adapters, perf fixes) should be offered to `paperclipai/paperclip` as PRs. See
"Syncing the Fork with Upstream" and "Pull Requests" below.

**graphify:**
- **Never graphify Paperclip itself.** Run graphify only on *project* code (e.g. UTMS). Paperclip is
  infrastructure, not a project to analyse. This follows from the no-self-modification rule.
- `graphify-out/` must live **outside the repository and outside the project workspace**. It is
  generated output: never tracked, never committed, never inside the project directory. A branch
  carrying hundreds of `graphify-out/` files is a mistake to be cleaned up, not merged.
...
Keep `atarim-info/paperclip` continuously current with `paperclipai/paperclip`. **Back up the
database first** — syncing pulls migrations, and applying them is not trivially reversible.

```bash
git remote -v                       # confirm origin=fork, upstream=paperclipai/paperclip
pnpm paperclipai db:backup          # ALWAYS before migrating

git fetch upstream
git rev-list --left-right --count origin/master...upstream/master   # fork-only / behind

git checkout master && git merge --ff-only upstream/master   # or a sync branch + PR if diverged
pnpm install && pnpm build
pnpm db:generate && pnpm db:migrate                          # if schema changed
```

**Pre-flight before ANY branch switch on the live checkout** — the systemd service runs from this
working tree, so a switch can take the control plane down:

1. `git show <target-ref>:package.json` — confirm the script systemd's `ExecStart` invokes still
   exists (currently `dev:once`). A missing script means an instant crash-loop.
2. Check `skills/` — `~/.claude/skills/*` are **symlinks into this repo**. A branch lacking a skill
   directory silently kills that skill.
3. Never leave the tree mid-rebase/mid-cherry-pick. If it conflicts, `--abort` immediately.
