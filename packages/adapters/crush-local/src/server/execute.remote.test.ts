...
const {
  ...
  // Crush `run` emits PLAIN TEXT; the session id + usage are recovered via a
  // second `crush session list --json` spawn.
  ...
  resolveCommandForLogs: vi.fn(async () => "ssh://fixture@127.0.0.1:2222/remote/workspace :: crush"),
...
describe("crush remote execution", () => {
  ...
  it("prepares the workspace, syncs Crush skills, recovers the session, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-crush-remote-"));
    ...
    // Crush needs no injected runtime config, so only the skills asset is synced.
    expect(syncDirectoryToSsh).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.paperclip-runtime/crush/skills`,
      followSymlinks: true,
    }));
  ...
  it("fails before the remote run when the configured model is unavailable on the SSH target", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-crush-remote-model-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    ).rejects.toThrow("Configured Crush model is unavailable on the remote execution target");
  ...
  it("resumes saved Crush sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-crush-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
