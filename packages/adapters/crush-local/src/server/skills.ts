...
function resolveCrushSkillsHome(config: Record<string, unknown>) {
  const env =
...
async function buildCrushSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    installedDetail: "Installed in the shared Claude/Crush skills home.",
    missingDetail: "Configured but not currently linked into the shared Claude/Crush skills home.",
    ...
    warnings: [
      "Crush currently uses the shared Claude skills home (~/.claude/skills).",
    ],
  });
}
...
export function resolveCrushDesiredSkillNames(
