...
type PreparedMimoRuntimeConfig = {
...
// gateway provider secrets (e.g. the LLM-gateway virtual key) into mimo.json
// SERVER-SIDE, where the value is reliably present. Mimo's own {env:...}
// resolution happens inside the (possibly sandboxed) run process, whose env
// plumbing is not guaranteed to carry the key to Mimo's spawned server -- so
// we resolve it here. Unresolvable placeholders are left intact for Mimo to try.
...
function parseProviderConfig(
  ...
  try {
    ...
    notes.push("PAPERCLIP_MIMO_PROVIDERS contains invalid JSON; custom providers ignored.");
    return null;
  }
  if (!isPlainObject(parsed)) {
    notes.push(
      "PAPERCLIP_MIMO_PROVIDERS is set but is not a JSON object; custom providers ignored.",
    );
    return null;
  }
  ...
  if (skipped.length > 0) {
    notes.push(
      `PAPERCLIP_MIMO_PROVIDERS: skipped provider(s) with non-object values: ${skipped.join(", ")}.`,
    );
  }
...
export async function prepareMimoRuntimeConfig(input: {
  env: Record<string, string>;
  config: Record<string, unknown>;
  targetIsRemote?: boolean;
}): Promise<PreparedMimoRuntimeConfig> {
  ...
  // env). Callers that need to ship a runtime mimo config to the remote
  ...
  const sourceConfigDir = path.join(resolveXdgConfigHome(input.env), "mimo");
  const runtimeConfigHome = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-mimo-config-"));
  const runtimeConfigDir = path.join(runtimeConfigHome, "mimo");
  const runtimeConfigPath = path.join(runtimeConfigDir, "mimo.json");
  ...
  const existingPermission = isPlainObject(existingConfig.permission)
    ...
    : {};
  const notes = [
    "Injected runtime Mimo config with permission.external_directory=allow to avoid headless approval prompts.",
  ];

  // Merge gateway/custom provider definitions supplied via PAPERCLIP_MIMO_PROVIDERS
  // (a JSON object in Mimo's `provider` shape). Mimo resolves a `--model
  // provider/model` only when that model exists in a provider's `models` map, and
  // MIMO_ALLOW_ALL_MODELS does NOT bypass its internal getModel(). So routing a
  ...
  const gatewayProviders = parseProviderConfig(
    input.env.PAPERCLIP_MIMO_PROVIDERS ?? process.env.PAPERCLIP_MIMO_PROVIDERS,
    resolveEnv,
    notes,
  );
  ...
  if (gatewayProviders) {
    notes.push(
      `Injected ${Object.keys(gatewayProviders).length} custom Mimo provider(s) from PAPERCLIP_MIMO_PROVIDERS: ${Object.keys(gatewayProviders).join(", ")}.`,
    );
  }

  // Register the configured model on its provider's models map. Mimo resolves
  ...
  // user config or PAPERCLIP_MIMO_PROVIDERS.
  ...
  if (configuredModel) {
    ...
    if (!isPlainObject(providerModels[configuredModel.model])) {
      ...
      notes.push(
        `Registered configured model ${configuredModel.provider}/${configuredModel.model} in the runtime Mimo config.`,
      );
    }
  }
  ...
  // Pin Mimo's auxiliary "small" model (used for session-title generation and
  // other helper tasks) via PAPERCLIP_MIMO_SMALL_MODEL. Mimo otherwise
  ...
  const smallModel = (input.env.PAPERCLIP_MIMO_SMALL_MODEL ?? process.env.PAPERCLIP_MIMO_SMALL_MODEL)?.trim();
  if (smallModel) {
    nextConfig.small_model = smallModel;
    notes.push(`Pinned Mimo small_model to ${smallModel}.`);
  }
