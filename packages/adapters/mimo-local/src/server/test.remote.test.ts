...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
      ...
      runtimeRootDir: "/remote/workspace/.paperclip-runtime/runs/test/workspace/.paperclip-runtime/mimo",
      assetDirs: {
        xdgConfig: "/remote/workspace/.paperclip-runtime/runs/test/workspace/.paperclip-runtime/mimo/xdgConfig",
      },
      restoreWorkspace,
    })),
  };
...
describe("mimo remote environment diagnostics", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("stages remote runtime config assets for sandbox hello probes", async () => {
    ...
    expect(runtimeInput?.adapterKey).toBe("mimo");
    ...
    expect(probeCall?.[4].env.XDG_CONFIG_HOME).toBe(
      "/remote/workspace/.paperclip-runtime/runs/test/workspace/.paperclip-runtime/mimo/xdgConfig",
    );
  });
});
