...
const {
  ...
  resolveCommandForLogs: vi.fn(async () => "ssh://fixture@127.0.0.1:2222/remote/workspace :: mimo"),
...
describe("mimo remote execution", () => {
  const cleanupDirs: string[] = [];
  const originalMimoAllowAllModels = process.env.MIMO_ALLOW_ALL_MODELS;
  ...
  afterEach(async () => {
    vi.clearAllMocks();
    if (originalMimoAllowAllModels === undefined) {
      delete process.env.MIMO_ALLOW_ALL_MODELS;
    } else {
      process.env.MIMO_ALLOW_ALL_MODELS = originalMimoAllowAllModels;
    }
  ...
  it("prepares the workspace, syncs Mimo skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-mimo-remote-"));
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.paperclip-runtime/mimo/xdgConfig`,
    }));
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.paperclip-runtime/mimo/skills`,
      followSymlinks: true,
    }));
    ...
    expect(modelProbeCall?.[3].env.XDG_CONFIG_HOME).toBe(
      `${managedRemoteWorkspace}/.paperclip-runtime/mimo/xdgConfig`,
    );
    ...
    expect(call?.[3].env.XDG_CONFIG_HOME).toBe(`${managedRemoteWorkspace}/.paperclip-runtime/mimo/xdgConfig`);
  ...
  it("fails before the remote run when the configured model is unavailable on the SSH target", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-mimo-remote-model-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    ).rejects.toThrow("Configured Mimo model is unavailable on the remote execution target");
  ...
  it("resumes saved Mimo sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-mimo-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
