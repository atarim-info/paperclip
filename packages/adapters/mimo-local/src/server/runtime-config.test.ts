...
async function makeConfigHome(initialConfig?: Record<string, unknown>) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-mimo-test-"));
  cleanupPaths.add(root);
  const configDir = path.join(root, "mimo");
  await fs.mkdir(configDir, { recursive: true });
  if (initialConfig) {
    await fs.writeFile(
      path.join(configDir, "mimo.json"),
...
describe("prepareMimoRuntimeConfig", () => {
  it("injects an external_directory allow rule by default", async () => {
    ...
    const prepared = await prepareMimoRuntimeConfig({
    ...
    const runtimeConfig = JSON.parse(
      await fs.readFile(
        path.join(prepared.env.XDG_CONFIG_HOME, "mimo", "mimo.json"),
        "utf8",
      ),
  ...
  it("merges custom providers from PAPERCLIP_MIMO_PROVIDERS into the config", async () => {
    ...
    const prepared = await prepareMimoRuntimeConfig({
      env: {
        ...
        PAPERCLIP_MIMO_PROVIDERS: JSON.stringify(providers),
      },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    ...
    const runtimeConfig = JSON.parse(
      await fs.readFile(path.join(prepared.env.XDG_CONFIG_HOME, "mimo", "mimo.json"), "utf8"),
    ) as Record<string, unknown>;
    expect(runtimeConfig).toMatchObject({
  ...
  it("reads PAPERCLIP_MIMO_PROVIDERS from process.env when absent from the run env", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const providers = { bifrost: { npm: "@ai-sdk/openai-compatible", models: { "example/model-a": {} } } };
    process.env.PAPERCLIP_MIMO_PROVIDERS = JSON.stringify(providers);
    try {
      const prepared = await prepareMimoRuntimeConfig({
        env: { XDG_CONFIG_HOME: configHome },
        config: {},
      ...
      delete process.env.PAPERCLIP_MIMO_PROVIDERS;
    }
  });
  ...
  it("expands {env:VAR} placeholders in custom providers using the run/process env (bakes the literal vk)", async () => {
    ...
    const prepared = await prepareMimoRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, PAPERCLIP_MIMO_PROVIDERS: JSON.stringify(providers), ANTHROPIC_API_KEY: "sk-bf-REALVK" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
    ...
    // The {env:...} placeholder must be replaced with the literal value, so Mimo
  ...
  it("leaves an unresolvable {env:VAR} placeholder intact", async () => {
    ...
    const prepared = await prepareMimoRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, PAPERCLIP_MIMO_PROVIDERS: JSON.stringify(providers) },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
  ...
  it("pins small_model from PAPERCLIP_MIMO_SMALL_MODEL", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const prepared = await prepareMimoRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, PAPERCLIP_MIMO_SMALL_MODEL: "example/model-a" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
  ...
  it("ignores malformed PAPERCLIP_MIMO_PROVIDERS without writing a provider block and surfaces a note", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const prepared = await prepareMimoRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, PAPERCLIP_MIMO_PROVIDERS: "not json" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
    ...
    expect(prepared.notes).toContain(
      "PAPERCLIP_MIMO_PROVIDERS contains invalid JSON; custom providers ignored.",
    );
    await prepared.cleanup();
  });

  it("surfaces a note when PAPERCLIP_MIMO_PROVIDERS is valid JSON but not an object", async () => {
    const configHome = await makeConfigHome({ permission: { read: "allow" } });
    const prepared = await prepareMimoRuntimeConfig({
      env: { XDG_CONFIG_HOME: configHome, PAPERCLIP_MIMO_PROVIDERS: "[1,2,3]" },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
    const runtimeConfig = JSON.parse(
  ...
  it("surfaces skipped provider entries with non-object values and keeps the usable ones", async () => {
    ...
    const prepared = await prepareMimoRuntimeConfig({
      env: {
        XDG_CONFIG_HOME: configHome,
        PAPERCLIP_MIMO_PROVIDERS: JSON.stringify({
    ...
    expect(prepared.notes).toContain(
      "PAPERCLIP_MIMO_PROVIDERS: skipped provider(s) with non-object values: bifrost.",
    );
    await prepared.cleanup();
  });
  ...
  it("surfaces skipped provider entries when no usable entries remain", async () => {
    ...
    const prepared = await prepareMimoRuntimeConfig({
      env: {
        XDG_CONFIG_HOME: configHome,
        PAPERCLIP_MIMO_PROVIDERS: JSON.stringify({ bifrost: "http://gateway.example/v1" }),
      },
      config: {},
    });
    cleanupPaths.add(prepared.env.XDG_CONFIG_HOME);
  ...
  it("registers a configured model missing from the catalog on its provider", async () => {
    ...
    expect(prepared.notes).toContain(
      "Registered configured model openrouter/openai/gpt-oss-120b:nitro in the runtime Mimo config.",
    );
    await prepared.cleanup();
  });
