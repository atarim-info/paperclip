...
import { discoverMimoModels, ensureMimoModelConfiguredAndAvailable } from "./models.js";
import { parseMimoJsonl } from "./parse.js";
...
const MIMO_AUTH_REQUIRED_RE =
  /(?:auth(?:entication)?\s+required|api\s*key|invalid\s*api\s*key|not\s+logged\s+in|mimo\s+auth\s+login|free\s+usage\s+exceeded)/i;
...
export async function testEnvironment(
  ...
  const targetLabel = targetIsRemote
    ...
    : null;
  const runId = `mimo-envtest-${Date.now()}-${Math.random().toString(16).slice(2)}`;

  if (targetLabel) {
    checks.push({
      code: "mimo_environment_target",
      level: "info",
  ...
  if (openaiKeyOverride !== null && openaiKeyOverride.trim() === "") {
    checks.push({
      code: "mimo_openai_api_key_missing",
      level: "warn",
  ...
  // Prevent Mimo from writing an mimo.json into the working directory.
  ...
  if (asBoolean(config.dangerouslySkipPermissions, true)) {
    checks.push({
      code: "mimo_headless_permissions_enabled",
      level: "info",
      message: "Headless Mimo external-directory permissions are auto-approved for unattended runs.",
    });
  }
  ...
  try {
    ...
    let runtimeCwd = cwd;
    if (targetIsRemote) {
      preparedRuntimeWorkspaceLocalDir = await fs.mkdtemp(path.join(os.tmpdir(), `paperclip-mimo-envtest-${runId}-`));
    ...
    const cwdInvalid = checks.some((check) => check.code === "mimo_cwd_invalid");
    if (cwdInvalid) {
      checks.push({
        code: "mimo_command_skipped",
        level: "warn",
      ...
      try {
        ...
        checks.push({
          code: "mimo_command_resolvable",
          level: "info",
          message: `Command is executable: ${command}`,
        });
      } catch (err) {
        checks.push({
          code: "mimo_command_unresolvable",
          level: "error",
    ...
    const canRunProbe =
      checks.every((check) => check.code !== "mimo_cwd_invalid" && check.code !== "mimo_command_unresolvable");

    let modelValidationPassed = false;
    const configuredModel = asString(config.model, "").trim();

    // Model discovery and validation use local child processes against
    // Mimo's `models` subcommand and JSON config; these are not yet
    ...
    if (targetIsRemote && configuredModel) {
      checks.push({
        code: "mimo_model_validation_skipped_remote",
        level: "info",
      ...
      try {
        const discovered = await discoverMimoModels({ command, cwd, env: runtimeEnv });
        if (discovered.length > 0) {
          checks.push({
            code: "mimo_models_discovered",
            level: "info",
            message: `Discovered ${discovered.length} model(s) from Mimo providers.`,
          });
        } else {
          checks.push({
            code: "mimo_models_empty",
            level: "error",
            message: "Mimo returned no models.",
            hint: "Run `mimo models` and verify provider authentication.",
          });
        }
      } catch (err) {
        ...
        if (/ProviderModelNotFoundError/i.test(errMsg)) {
          checks.push({
            code: "mimo_hello_probe_model_unavailable",
            level: "warn",
            message: "The configured model was not found by the provider.",
            detail: errMsg,
            hint: "Run `mimo models` and choose an available provider/model ID.",
          });
        } else {
          checks.push({
            code: "mimo_models_discovery_failed",
            level: "error",
            message: errMsg || "Mimo model discovery failed.",
            hint: "Run `mimo models` manually to verify provider auth and config.",
          });
        }
      }
      ...
      try {
        ...
        if (/ProviderModelNotFoundError/i.test(errMsg)) {
          ...
          checks.push({
            code: "mimo_models_discovery_failed",
            level: "warn",
            message: errMsg || "Mimo model discovery failed (best-effort, no model configured).",
            hint: "Run `mimo models` manually to verify provider auth and config.",
          });
        }
      }
    ...
    const modelUnavailable = checks.some((check) => check.code === "mimo_hello_probe_model_unavailable");
    if (!configuredModel && !modelUnavailable) {
      ...
      try {
        ...
        checks.push({
          code: "mimo_model_invalid",
          level: "error",
          message: err instanceof Error ? err.message : "Configured model is unavailable.",
          hint: "Run `mimo models` and choose a currently available provider/model ID.",
        });
      }
    }
    ...
    if (canRunProbe && modelValidationPassed) {
      ...
      try {
        ...
        const parsed = parseMimoJsonl(probe.stdout);
        const detail = summarizeProbeDetail(probe.stdout, probe.stderr, parsed.errorMessage);
        const authEvidence = `${parsed.errorMessage ?? ""}\n${probe.stdout}\n${probe.stderr}`.trim();

        if (probe.timedOut) {
          checks.push({
            code: "mimo_hello_probe_timed_out",
            level: "warn",
            message: "Mimo hello probe timed out.",
            hint: "Retry the probe. If this persists, run Mimo manually in this working directory.",
          });
        } else if ((probe.exitCode ?? 1) === 0 && !parsed.errorMessage) {
          const summary = parsed.summary.trim();
          const hasHello = /\bhello\b/i.test(summary);
          checks.push({
            code: hasHello ? "mimo_hello_probe_passed" : "mimo_hello_probe_unexpected_output",
            level: hasHello ? "info" : "warn",
            message: hasHello
              ? "Mimo hello probe succeeded."
              : "Mimo probe ran but did not return `hello` as expected.",
        ...
        } else if (MIMO_AUTH_REQUIRED_RE.test(authEvidence)) {
          checks.push({
            code: "mimo_hello_probe_auth_required",
            level: "warn",
            message: "Mimo is installed, but provider authentication is not ready.",
            ...(detail ? { detail } : {}),
            hint: "Run `mimo auth login` or set provider credentials, then retry the probe.",
          });
        } else {
          checks.push({
            code: "mimo_hello_probe_failed",
            level: "error",
            message: "Mimo hello probe failed.",
            ...(detail ? { detail } : {}),
