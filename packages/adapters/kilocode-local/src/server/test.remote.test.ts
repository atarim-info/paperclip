...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
      ...
      runtimeRootDir: "/remote/workspace/.paperclip-runtime/runs/test/workspace/.paperclip-runtime/kilo",
      assetDirs: {
        xdgConfig: "/remote/workspace/.paperclip-runtime/runs/test/workspace/.paperclip-runtime/kilo/xdgConfig",
      },
      restoreWorkspace,
    })),
  };
...
describe("kilo remote environment diagnostics", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("stages remote runtime config assets for sandbox hello probes", async () => {
    ...
    expect(runtimeInput?.adapterKey).toBe("kilo");
    ...
    expect(probeCall?.[4].env.XDG_CONFIG_HOME).toBe(
      "/remote/workspace/.paperclip-runtime/runs/test/workspace/.paperclip-runtime/kilo/xdgConfig",
    );
  });
});
