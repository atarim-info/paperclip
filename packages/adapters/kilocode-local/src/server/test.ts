...
import { discoverKiloModels, ensureKiloModelConfiguredAndAvailable } from "./models.js";
import { parseKiloJsonl } from "./parse.js";
...
export async function testEnvironment(
  ...
  const targetLabel = targetIsRemote
    ...
    : null;
  const runId = `kilo-envtest-${Date.now()}-${Math.random().toString(16).slice(2)}`;

  if (targetLabel) {
    checks.push({
      code: "kilo_environment_target",
      level: "info",
  ...
  if (openaiKeyOverride !== null && openaiKeyOverride.trim() === "") {
    checks.push({
      code: "kilo_openai_api_key_missing",
      level: "warn",
  ...
  // Prevent Kilo from writing an kilo.json into the working directory.
  ...
  if (asBoolean(config.dangerouslySkipPermissions, true)) {
    checks.push({
      code: "kilo_headless_permissions_enabled",
      level: "info",
      message: "Headless Kilo external-directory permissions are auto-approved for unattended runs.",
    });
  }
  ...
  try {
    ...
    let runtimeCwd = cwd;
    if (targetIsRemote) {
      preparedRuntimeWorkspaceLocalDir = await fs.mkdtemp(path.join(os.tmpdir(), `paperclip-kilo-envtest-${runId}-`));
    ...
    const cwdInvalid = checks.some((check) => check.code === "kilo_cwd_invalid");
    if (cwdInvalid) {
      checks.push({
        code: "kilo_command_skipped",
        level: "warn",
      ...
      try {
        ...
        checks.push({
          code: "kilo_command_resolvable",
          level: "info",
          message: `Command is executable: ${command}`,
        });
      } catch (err) {
        checks.push({
          code: "kilo_command_unresolvable",
          level: "error",
    ...
    const canRunProbe =
      checks.every((check) => check.code !== "kilo_cwd_invalid" && check.code !== "kilo_command_unresolvable");

    let modelValidationPassed = false;
    const configuredModel = asString(config.model, "").trim();

    // Model discovery and validation use local child processes against
    // Kilo's `models` subcommand and JSON config; these are not yet
    ...
    if (targetIsRemote && configuredModel) {
      checks.push({
        code: "kilo_model_validation_skipped_remote",
        level: "info",
      ...
      try {
        const discovered = await discoverKiloModels({ command, cwd, env: runtimeEnv });
        if (discovered.length > 0) {
          checks.push({
            code: "kilo_models_discovered",
            level: "info",
            message: `Discovered ${discovered.length} model(s) from Kilo providers.`,
          });
        } else {
          checks.push({
            code: "kilo_models_empty",
            level: "error",
            message: "Kilo returned no models.",
        ...
        if (/ProviderModelNotFoundError/i.test(errMsg)) {
          checks.push({
            code: "kilo_hello_probe_model_unavailable",
            level: "warn",
          ...
          checks.push({
            code: "kilo_models_discovery_failed",
            level: "error",
            message: errMsg || "Kilo model discovery failed.",
      ...
      try {
        ...
        if (/ProviderModelNotFoundError/i.test(errMsg)) {
          ...
          checks.push({
            code: "kilo_models_discovery_failed",
            level: "warn",
            message: errMsg || "Kilo model discovery failed (best-effort, no model configured).",
            hint: "Run `kilo models` manually to verify provider auth and config.",
          });
        }
      }
    ...
    const modelUnavailable = checks.some((check) => check.code === "kilo_hello_probe_model_unavailable");
    ...
    if (canRunProbe && modelValidationPassed) {
      ...
      try {
        ...
        const parsed = parseKiloJsonl(probe.stdout);
        const detail = summarizeProbeDetail(probe.stdout, probe.stderr, parsed.errorMessage);
        const authEvidence = `${parsed.errorMessage ?? ""}\n${probe.stdout}\n${probe.stderr}`.trim();

        if (probe.timedOut) {
          checks.push({
            code: "kilo_hello_probe_timed_out",
            level: "warn",
            message: "Kilo hello probe timed out.",
            hint: "Retry the probe. If this persists, run Kilo manually in this working directory.",
          });
        } else if ((probe.exitCode ?? 1) === 0 && !parsed.errorMessage) {
          const summary = parsed.summary.trim();
          const hasHello = /\bhello\b/i.test(summary);
          checks.push({
            code: hasHello ? "kilo_hello_probe_passed" : "kilo_hello_probe_unexpected_output",
            level: hasHello ? "info" : "warn",
            message: hasHello
              ? "Kilo hello probe succeeded."
              : "Kilo probe ran but did not return `hello` as expected.",
            ...(summary ? { detail: summary.replace(/\s+/g, " ").trim().slice(0, 240) } : {}),
            ...(hasHello
              ? {}
              : {
                  hint: "Run `kilo run --format json` manually and prompt `Respond with hello` to inspect output.",
                }),
          });
        ...
        } else if (KILO_AUTH_REQUIRED_RE.test(authEvidence)) {
          checks.push({
            code: "kilo_hello_probe_auth_required",
            level: "warn",
            message: "Kilo is installed, but provider authentication is not ready.",
            ...(detail ? { detail } : {}),
            hint: "Run `kilo auth login` or set provider credentials, then retry the probe.",
          });
        } else {
          checks.push({
            code: "kilo_hello_probe_failed",
            level: "error",
            message: "Kilo hello probe failed.",
            ...(detail ? { detail } : {}),
            hint: "Run `kilo run --format json` manually in this working directory to debug.",
          });
        }
      } catch (err) {
        checks.push({
