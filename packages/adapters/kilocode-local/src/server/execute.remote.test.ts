...
const {
  ...
  resolveCommandForLogs: vi.fn(async () => "ssh://fixture@127.0.0.1:2222/remote/workspace :: kilo"),
...
describe("kilo remote execution", () => {
  const cleanupDirs: string[] = [];
  const originalKiloAllowAllModels = process.env.KILO_ALLOW_ALL_MODELS;
  ...
  afterEach(async () => {
    vi.clearAllMocks();
    if (originalKiloAllowAllModels === undefined) {
      delete process.env.KILO_ALLOW_ALL_MODELS;
    } else {
      process.env.KILO_ALLOW_ALL_MODELS = originalKiloAllowAllModels;
    }
  ...
  it("prepares the workspace, syncs Kilo skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-kilo-remote-"));
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.paperclip-runtime/kilo/xdgConfig`,
    }));
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.paperclip-runtime/kilo/skills`,
      followSymlinks: true,
    }));
    ...
    expect(modelProbeCall?.[3].env.XDG_CONFIG_HOME).toBe(
      `${managedRemoteWorkspace}/.paperclip-runtime/kilo/xdgConfig`,
    );
    ...
    expect(call?.[3].env.XDG_CONFIG_HOME).toBe(`${managedRemoteWorkspace}/.paperclip-runtime/kilo/xdgConfig`);
  ...
  it("fails before the remote run when the configured model is unavailable on the SSH target", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-kilo-remote-model-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    ).rejects.toThrow("Configured Kilo model is unavailable on the remote execution target");
  ...
  it("resumes saved Kilo sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-kilo-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
